...
class LayoutRow<T> {
  ...
  isRangeClear(left: number, right: number) {
    ...
    const startX = Math.max(min, left) - offset

    // Early exit optimization: check every Nth element first
    const step = Math.max(1, Math.floor((maxX - startX) / 10))
  ...
  addRect(rect: Rectangle<T>, data: string): void {
    ...
    // Optimized filling - use Array.fill for larger ranges
    if (w < 50) {
      for (let x = oLeft; x < oRight; x += 1) {
        this.row.bits[x] = data
      }
    } else {
      this.row.bits.fill(data, oLeft, oRight)
    }
...
export default class GranularRectLayoutRowHints<T> implements BaseLayout<T> {
  ...
  // Row hint optimization: track the minimum row where we might find space
  ...
  addRect(
    ...
    if (this.displayMode !== 'collapse') {
      // Start searching from the hint instead of 0
      ...
      // This helps avoid unnecessary scanning
