...
// V3: Use Int32Array for intervals - better memory layout and cache
class V3Row {
  allFilled = false
  // Int32Array for better cache locality  intervals: Int32Array
  len = 0
  ...
  // Inline-friendly collision check
...
export default class GranularRectLayoutV3<T> implements BaseLayout<T> {
  private pitchX: number
  private hardRowLimit: number
  private bitmap: V3Row[]
  ...
  private autovivifyRow(bitmap: V3Row[], y: number) {
    let row = bitmap[y]
    if (!row) {
